...
const meal_ingr = fetch(`${baseUrl}commonIngredients`, {
  method: "POST",
  headers: {
  ...
  // console.log(data);
  return data.ingredients.map(item => item);
})
...
async function selectAction(action, cnt){
    ...
    // Back to default from Add 
        ...
        Array.from(document.getElementsByClassName("checkBox")).forEach(box => 
    ...
    // This is adding items when the add button is clicked
        ...
        if (ingr.includes(x)) {
            const resp = await fetch(`${baseUrl}addDrinkItem/${x}`, {
                method: "POST",
                headers: {
                    'Content-Type' : 'application/json'
                }
    ...
    // Removing items from the inventory
        ...
        let inventory = document.querySelectorAll('.checkBox');
        let toRemove = [];
        ...
        const resp = await fetch(`${baseUrl}removeDrinkItems/${json}`, {
                method: "POST",
                headers: {
                    'Content-Type' : 'application/json'
            }
...
async function selectFoodAction(action, cnt){
    // Remove Action
    let meal_ingredients = await meal_ingr;
    if(action === 1 ){
        // Remove the default add and remove buttons
        document.getElementById("addInven").style.visibility = 'hidden';
        document.getElementById("rmInven").style.visibility = 'hidden';
    ...
    // Add action
        ...
        document.getElementById("rmInven").style.visibility = 'hidden';
        // Showcase the input bar and submit button
        document.getElementById("foodInput").classList.toggle('hidden');
        document.getElementById("backBtn").classList.toggle('hidden');
        document.getElementById("submitItem").classList.toggle('hidden');
    // Back to default from Add 
    } else if (action === 2){
    ...
    // This is adding items when the add button is clicked
        ...
        let x = document.getElementById("foodInput").value;
        let res = meal_ingredients.filter((y) => {
            return x == y.item;
        })
        console.log(res);
        if (res.length > 0) {
            const resp = await fetch(`${baseUrl}addRecipeItem/${x}`, {
                method: "POST",
                headers: {
                    'Content-Type' : 'application/json'
                }
            ...
            const data = await resp.json();
            if(data.success == 200){
                const node = document.getElementById("foodInventory");
                node.innerHTML = data.data;    
            } else if (data.success == -1){
                document.getElementById("itemAlert").classList.toggle('hidden');
                document.getElementById("itemAlert").innerHTML = `<span class="absolute right-3" onclick="this.parentElement.style.display='none';">&times;</span>
                                    ${x} was already found in your inventory. No duplicates are allowed.`;
            }
        } else {
            // Checks to see if the error alert is visible already. if it is vis alreay then do nothing 
            if(!document.getElementById("itemAlert").classList.contains('hidden')){
    ...
    // Removing items from the inventory
        ...
        const json = JSON.stringify(toRemove);
        console.log(json);
        const resp = await fetch(`${baseUrl}removeRecipeItem/${json}`, {
                method: "POST",
                headers: {
                    'Content-Type' : 'application/json'
            }
