...
app.post('/login', async (req,res) => {
    ...
    if(r){
        ...
        req.session.favorites = new Set(r.drinkProfile.favorites);
    ...
    Promise.all([...promiseList])
        .then(results => {
            ...
            results.forEach(drink => {
                ...
                COCKTAIL_CATEGORIES.add(drinks.strDrink);
                favs += "</tr>";
            })
            let entries = "";
            let inventory = "";
...
app.post('/processFilters', (req,res)=>{
    ...
    Promise.all([...promiseList]).then(
        results => {
            ...
            COCKTAIL_CATEGORIES.forEach(e => {categories+=`<option value="${e}">${e}</option>`;});
            let favorites = ""
            results.forEach(r => {
                drink = r.drinks[0];
...
app.get('/drinks/:id', async (req,res) =>{
    ...
    try {
        await client.connect();
        let query = {_id: req.session.user}
        let recent = {$push: {'drinkProfile.recents': id}}
    ...
    .then( r => {
        ...
        // console.log(drink);
        let favBool = req.session.favorites.has(id);
        ...
        // let ls = instr.split(".");
        // console.log(instr);
        res.render('customDrink',{user: req.session.user, id:id.id, drinkName: name, image:imgUrl, instructions:instr, ingredients:ingList, fav: favBool} );  
    }).catch(e => {
        console.log(e);
    })
})
...
app.get('/addFavoriteDrink/:id', async (req,res) => {
    ...
    if(req.session.favorites.has(id)){
        try {
            await client.connect();
            let query = {_id: req.session.user}
        ...
        try {
            await client.connect();
            let query = {_id: req.session.user}
            let newFav = {$pull: {'drinkProfile.favorites': id}}
            r = await client.db(MONGO_DB_NAME).collection('users').updateOne(query,newFav);
            console.log(`Application entry created with id ${r.id}`);
        } catch (e){
            console.log(e)
    ...
    .then( r => {
        ...
        let drink = d.drinks[0];
        // console.log(drink);
        let fav = req.session.favorites.has(id);
        let instr = `<span>${drink.strInstructions}</span>`;
        let img = drink.strDrinkThumb;
        let imgUrl = `<img src=${img} class='drinkImage'>`
        let name = drink.strDrink;
